/*
 * File: serial.c
 * Author: Weiyang Wang <wew168@ucsd.edu>, with some copied from stackoverflow
...
unsigned int serial_init(serial ** spp) {
    ...
    if (!spp) {
        fprintf(stderr, "ERROR in init: "
            "null pointer passed for initialization.\n");
        return FALSE;
    }

    // *spp needs to be null to proceed for safety
    if (*spp) {
        fprintf(stderr, "ERROR in init: object already initialized.\n");
        return FALSE;
    }

    serial * ser = (serial*)malloc(sizeof(serial));
    if (!ser) {
        fprintf(stderr, "ERROR in init: memory allocation failed.\n");
        return FALSE;
    }
